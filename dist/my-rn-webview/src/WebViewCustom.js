 ...
 */
export async function webProcessHtml(html, ROOT_RESOURCE, dirCacheImage) {
    if (html == null)
        return null;
...
function isCache(subPath, dirCache) {
    if (dirCache == null)
        return false;
    return FileUtils.exists(dirCache + subPath);
}
export class WebViewCustom extends Component {
    constructor(props) {
        super(props);
        this.id = 0;
    ...
    renderContent() {
        ...
        if (this.props.isAutoHeightWebView) {
            return (<AutoHeightWebView {...webProps} onHeightUpdated={(hight) => { console.log("onHeightUpdated: ", hight); }} enableAnimation={false}/>);
        }
        else
            return (<WebView {...webProps}/>);
    }
    render() {
        this.id++;
        return (<VContainerLoad ref={(ref) => { this.vContainer = ref; }} id={this.id} loadDataAsync={this.loadDataAsync.bind(this)} onRender={this.renderContent.bind(this)}/>);
    }
}
WebViewCustom.defaultProps = {
